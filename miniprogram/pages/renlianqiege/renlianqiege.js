...
Page({
  ...
  sertIntoDB(){
    var that=this
    db.collection('CG_renlianqiege').add({
  ...
  getToken(){
    var that=this
    wx.request({
      url: 'https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id=XSBkzButyFydCzFShhydmIqn&client_secret=tr4vBgrYzsjrbYrWSaAnduEUH5AEVOhI&',
      method:'post',
      success:(res)=>{
  ...
  sendPost(){
    ...
    wx.request({
      ...
      success:(res)=>{
        console.log(res)
        if(res.statusCode===200 && res.data.foreground){
          that.setData({
            foreground:'data:image/png;base64,' + res.data.foreground
          })
          wx.hideLoading({
            success: (res) => {},
          })
